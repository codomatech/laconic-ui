var $interface = (function() {
  ...
  const table = async (values, key) => {
    $interface.state.dsources[values.datasource] = $interface.state.dsources[
      values.datasource
    ] || { display: [], raw: [] };
      ...
      methods: {
        showeditdialog: function(index) {
          ...
          $interface.bus.emit("gui", {
            ...
            args: [
              { display: tabledata.display[index], raw: tabledata.raw[index] }
            ]
          });
        }
      },
    ...
    computed: {
      header() {
        return Store.state.staticComponents.header
          ? Store.state.staticComponents.header
          : {};
      }
    }
  });
    ...
    computed: {
      footer() {
        return Store.state.staticComponents.footer
          ? Store.state.staticComponents.footer
          : {};
      }
    }
  });
          ...
          let obj = {
            ...
            component: Vue.component(`app-${ID}`, {
              ...
              methods: {
                ...
                submitForm(d) {
                  let resultObject = this.PageData.reduce(
  ...
  function registerbusevents($interface) {
    ...
    // handle http request
    $interface.bus.on("subscriptions-sendsignedrequest", function(e) {
      var t =
        u.subscriptionrole == "principal"
          ? c + " " + u.customerid + " " + a(u.password)
          : c + " " + u.customerid + " " + a(u.password) + " " + u.email;
      fetch(e.url, {
        method: e.method,
        headers: {
            ...
            data.actions.forEach(function(action) {
              modal.addFooterBtn(action.title, "tingle-btn", function() {
                typeof action.callback === "function" &&
                  action.callback.apply(null, data.args || []);
                modal.close();
              });
            });
